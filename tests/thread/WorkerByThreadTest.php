...
class WorkerByThreadTest extends TestCase
{
  ...
  {
    if (!\IS_THREADED_UV)
      $this->markTestSkipped('Needs `ZTS` versions of PHP!');
  ...
  {
    $this->markTestSkipped('The `run_in_thread()` function needs refactoring.');
    $this->results = [];
  ...
  public function test_bad_cpu()
  {
    $this->markTestSkipped('The `run_in_thread()` function needs refactoring.');
    async('main', function () {
      yield test_raises_async($this, '\TypeError', run_in_thread, 'fib', 'bad1');
    });
  ...
  public function test_worker_cancel()
  {
    ...
    async('blocking', function ($n) {
      $task = yield spawner(run_in_thread, 'sleep', $n);
...
10, 9, 8, 7, 6, 5, 'cancel', 4, 3, 2, 1
  ...
  public function test_worker_timeout()
  {
    ...
    async('blocking', function ($n) {
      try {
        $result = yield timeout_after(0.25, run_in_thread, 'sleep', $n);
